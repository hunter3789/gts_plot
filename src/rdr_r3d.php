...
$lat1  = $_REQUEST["lat1"];    if ($lat1 == "") $lat1 = "36";
$lon1  = $_REQUEST["lon1"];    if ($lon1 == "") $lon1 = "124";
$lat2  = $_REQUEST["lat2"];    if ($lat2 == "") $lat2 = "38";
$lon2  = $_REQUEST["lon2"];    if ($lon2 == "") $lon2 = "128";
$opt = $tm.",".$lat1.",".$lon1.",".$lat2.",".$lon2;
?>

<HTML>
<HEAD>
<title>레이더 3차원 단면도</title>
...
<script type="text/javascript" src="./rdr_r3d.js?<?=date('Ymdhis')?>"></script>
    ...
    <tr>
      <td>
        ...
        <tr>
          ...
          <td nowrap width=1></td> 
          <td nowrap class=TB09 style="background-color:#f3fcff;" onmouseup="tm_move('-5m');">-5m</td>
          <td nowrap width=1></td> 
          <td nowrap class=TB09 style="background-color:#fff4f1;" onmouseup="tm_move('+5m');">+5m</td>
          <td nowrap width=1></td> 
          ...
          <td nowrap><input type="checkbox" id="layer01" name="layer_list" onclick="doSubmit();" value="W" checked><label for="layer01" class=text1 style='position:relative; top:-3px;'>+바람</label></td>

          <td nowrap width=5></td>
          <td nowrap><input type="checkbox" id="layer02" name="layer_list" onclick="doSubmit();" value="L"><label for="layer02" class=text1 style='position:relative; top:-3px;'>+낙뢰</label></td>
...
<div id='body' style='overflow:auto;'>
  <div style='display:flex;'>
    <div id=map_container style='min-width:515px;'>
      <div class=text1>레이더 이미지를 클릭하여 단면의 위치를 조정할 수 있습니다.</div>  
      <div><img id=rdr_map onmousedown='fnClickImg();'></div>
    </div>

    <div id=r3d_container>
      <div><img id=r3d_img1></div>
      <div><img id=r3d_img2></div>
    </div>
  </div>
</div>
